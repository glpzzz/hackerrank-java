...
class ProductOfRedNodesVisitor extends TreeVis {
    ...
    void multiply(Tree tree) {
        if (tree.getColor() == Color.RED)
            this.value = (this.value * tree.getValue()) % (1000000007);
    }
...
class FancyVisitor extends TreeVis {
    ...
    int evenDepthNodesSum = 0;
    int greenLeafSum = 0;

    public int getResult() {
        return Math.abs(this.greenLeafSum-this.evenDepthNodesSum);
    }

    public void visitNode(TreeNode node) {
        if(node.getDepth() % 2 != 0) return;
        this.evenDepthNodesSum += node.getValue();
    }
